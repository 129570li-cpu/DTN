...
USE_GNN_ROUTER = os.environ.get("ELITE_USE_GNN", "0") == "1"
GNN_MODEL_DIR = os.environ.get("ELITE_GNN_MODEL_DIR")
GNN_DEVICE = os.environ.get("ELITE_GNN_DEVICE", "cpu")
GNN_HIDDEN_DIM = int(os.environ.get("ELITE_GNN_HIDDEN", "64"))
TRAFFIC_BASE_SPEED = float(os.environ.get("ELITE_TRAFFIC_BASE_SPEED", "23.0"))
TRAFFIC_MAX_QUEUE = float(os.environ.get("ELITE_TRAFFIC_MAX_QUEUE", "30.0"))
TRAFFIC_MAX_NEIGHBOR = float(os.environ.get("ELITE_TRAFFIC_MAX_NEIGHBOR", "20.0"))
...
try:
    ...
    GraphConverter = None  # type: ignore
    FedG_DQN = None  # type: ignore
...
class GNNRouter:
    """
    Lightweight wrapper around GraphSAGE+DQN model for controller inference.
    """
    ...
    def __init__(self,
                 ...
                 hidden_dim: int = 64,
                 device: str = "cpu"):
        if torch is None or GraphConverter is None or FedG_DQN is None:
        ...
        with open(mapping_path, "r") as f:
            raw = json.load(f) if isinstance(mapping_path, str) else {}
        id_to_idx = {int(k): int(v) for k, v in raw.items()}
        idx_to_id = {v: k for k, v in id_to_idx.items()}
        self.converter.id_to_idx = id_to_idx
        self.converter.idx_to_id = idx_to_id
        self.converter.num_nodes = len(id_to_idx)
        data = self.converter.build_graph(it_pos, adj, node_features=None)
        self.graph_data = data.to(device)
        in_dim = self.graph_data.x.size(1)
        self.dynamic_offset = 3  # [normX, normY, degree]
        self.dynamic_dim = max(0, in_dim - self.dynamic_offset)
        self.model = FedG_DQN(in_dim, hidden_dim).to(device)
        state = torch.load(model_path, map_location=device)
        self.model.load_state_dict(state)
        self.model.eval()

    def has_node(self, node_id: int) -> bool:
        return self.converter.get_idx(node_id) >= 0

    def compute_path(self, src: int, dst: int, max_hops: int = 128) -> List[int]:
        ...
        def dfs(curr: int, depth: int) -> bool:
            if curr == dst:
                return True
            if depth >= max_hops:
                dead_ends.add(curr)
            ...
            candidates = [
                ...
                if self.has_node(n) and n not in visited and n not in dead_ends
            ]
            if not candidates:
                dead_ends.add(curr)
                return False
            curr_idx = self.converter.get_idx(curr)
            dest_idx = self.converter.get_idx(dst)
            neighbor_idx = [self.converter.get_idx(n) for n in candidates]
            with torch.no_grad():
                scores = self.model(
                    self.graph_data.x,
            ...
            if scores.numel() == 0:
                ...
                return False
            ordering = sorted(
                zip(candidates, scores.cpu().tolist()),
                key=lambda kv: kv[1],
                reverse=True,
            )
            for nxt, _ in ordering:
                path.append(nxt)
                visited.add(nxt)
                if dfs(nxt, depth + 1):
                    return True
    ...
    def update_node_features(self, updates: Dict[int, Tuple[float, float, float]]) -> None:
        if self.dynamic_dim <= 0 or not updates:
            return
        for node_id, feat in updates.items():
            idx = self.converter.get_idx(int(node_id))
            if idx < 0:
                continue
            values = list(feat)
            if len(values) < self.dynamic_dim:
                values.extend([0.0] * (self.dynamic_dim - len(values)))
            tensor_vals = torch.tensor(values[:self.dynamic_dim],
                                       dtype=self.graph_data.x.dtype,
                                       device=self.graph_data.x.device)
            self.graph_data.x[idx, self.dynamic_offset:self.dynamic_offset + self.dynamic_dim] = tensor_vals


def maybe_load_gnn_router(it_pos: Dict[int, Tuple[float, float]],
                          adj: Dict[int, List[int]]) -> Optional[GNNRouter]:
    if not USE_GNN_ROUTER:
        return None
    if not GNN_MODEL_DIR:
        _ctrl_log("ELITE_USE_GNN enabled but ELITE_GNN_MODEL_DIR not set.")
        return None
    if torch is None or GraphConverter is None or FedG_DQN is None:
        _ctrl_log("GNN router requested but PyTorch/Graph modules unavailable.")
        return None
    model_path = os.path.join(GNN_MODEL_DIR, "fedg_dqn.pt")
    mapping_path = os.path.join(GNN_MODEL_DIR, "id_mapping.json")
    if not os.path.exists(model_path) or not os.path.exists(mapping_path):
        _ctrl_log(f"GNN router files not found in {GNN_MODEL_DIR}")
        return None
    try:
        router = GNNRouter(model_path, mapping_path, it_pos, adj,
                           hidden_dim=GNN_HIDDEN_DIM, device=GNN_DEVICE)
        _ctrl_log("GNN router loaded successfully.")
        return router
    except Exception as exc:
        _ctrl_log(f"Failed to initialize GNN router: {exc}")
        return None
...
def prepare_topology_and_policies(net_xml_path: str,
    ...
    if USE_GNN_ROUTER:
        gnn_router = maybe_load_gnn_router(it_pos, adj)
        if not gnn_router:
            raise RuntimeError("GNN router requested but unavailable.")
        ctrl.enable_gnn_router(True)
        ctrl.set_gnn_router(gnn_router)
...
class TrafficFeatureCache:
    """
    Maintains latest queue/neighbor metrics per junction and converts them to
    GNN dynamic features (speed, density, queue). Speed is heuristically
    estimated based on congestion level so the trained model can reuse the
    same feature layout as离线训练.
    """
    def __init__(self,
                 base_speed: float = TRAFFIC_BASE_SPEED,
                 max_queue: float = TRAFFIC_MAX_QUEUE,
                 max_neighbor: float = TRAFFIC_MAX_NEIGHBOR):
        self.base_speed = max(0.0, base_speed)
        self.max_queue = max(1e-3, max_queue)
        self.max_neighbor = max(1e-3, max_neighbor)
        self.state: Dict[int, Dict[str, float]] = {}

    def update(self, area_id: int, queue_len: float, neighbor_count: float,
               buffer_bytes: Optional[float] = None) -> Optional[Tuple[float, float, float]]:
        if area_id is None:
            return None
        entry = self.state.setdefault(int(area_id), {})
        entry["queue"] = max(0.0, float(queue_len))
        entry["neighbor"] = max(0.0, float(neighbor_count))
        if buffer_bytes is not None:
            entry["buffer"] = max(0.0, float(buffer_bytes))
        return self._to_features(entry)

    def _to_features(self, entry: Dict[str, float]) -> Tuple[float, float, float]:
        queue = min(entry.get("queue", 0.0), self.max_queue)
        density = min(entry.get("neighbor", 0.0), self.max_neighbor)
        # Congestion score merges queue与邻居数
        load = 0.5 * (queue / self.max_queue + density / self.max_neighbor)
        load = max(0.0, min(1.0, load))
        speed = max(0.0, self.base_speed * (1.0 - load))
        return (speed, density, queue)
    ...
    try:
        ...
        while True:
            ...
            with rl as data:
                ...
                # 更新交通动态特征以便 GNN 关注实时拥堵
                if ctrl.gnn_router_enabled and getattr(ctrl, "gnn_router", None):
                    src_area_dyn = ctrl.get_area_id(srcId)
                    dyn_feat = traffic_cache.update(src_area_dyn, queueLen, neighborCount, bufferBytes)
                    if dyn_feat:
                        ctrl.gnn_router.update_node_features({src_area_dyn: dyn_feat})
            ...
            if routeRequestFlag == 1:
                ...
                with policy_lock:
                    ...
                    if not area_path:
                        _ctrl_log(f"GNN path unavailable for src={srcId} dst={dstId}; dropping request.")
                        data.act.path_len = 0
                        continue
