...
class EnvConfig:
    ...
    seed: int = 1
    queue_depart_rate: float = 0.5  # vehicles per second departing a queue
    density_hop_scale: float = 0.5  # scaling factor for hop success probability
...
class DTNEnv:
    def __init__(self,
                 ...
                 density_map: Optional[Dict[int, Dict[int, float]]] = None,
                 node_features: Optional[Dict[int, List[float]]] = None) -> None:
        self.it_pos = it_pos
        self.adj = adj
        self.cfg = cfg
        ...
        # per-node dynamic stats (avg speed, density, queue length)
        self.node_speed: Dict[int, float] = {}
        self.node_density: Dict[int, float] = {}
        self.node_queue: Dict[int, float] = {}
        self.set_node_features(node_features)
        # approximate max path delay baseline for normalization
        self.max_pair_delay = self._estimate_max_pair_delay()

    def set_node_features(self, features: Optional[Dict[int, List[float]]]):
        self.node_speed.clear()
        self.node_density.clear()
        self.node_queue.clear()
        if not features:
            return
        for nid, feats in features.items():
            node_id = int(nid)
            vals = list(feats) + [0.0, 0.0, 0.0]
            self.node_speed[node_id] = float(vals[0])
            self.node_density[node_id] = max(0.0, float(vals[1]))
            self.node_queue[node_id] = max(0.0, float(vals[2]))
    ...
    def seg_delay(self, c: int, a: int) -> float:
        v = self.speed_map.get(c, {}).get(a)
        if v is None or v <= 0.0:
            v = self.node_speed.get(c) or self.node_speed.get(a) or self.cfg.veh_speed
        travel = self.seg_length(c, a) / max(1e-3, v)
        queue_penalty = self._queue_delay(c) + 0.5 * self._queue_delay(a)
        return travel + queue_penalty
    ...
    def seg_ctrl_overhead(self, c: int, a: int) -> float:
        ...
        dens = self.density_map.get(c, {}).get(a)
        if dens is None:
            dens = 0.5 * (self.node_density.get(c, 0.0) + self.node_density.get(a, 0.0))
    ...
    def seg_delivery_prob(self, c: int, a: int) -> float:
        # 密度因子：过稀/过密都差，中间最优
        dens = 0.5 * (self.node_density.get(c, 0.0) + self.node_density.get(a, 0.0))
        scale = max(1e-3, self.cfg.density_hop_scale)
        x = dens / scale
        # 如果没有密度数据，使用默认成功率；否则按密度曲线计算
        if dens <= 0.0:
            dens_factor = 0.7  # 默认 70% 基础成功率
        else:
        dens_factor = max(0.0, min(1.0, x * math.exp(1 - x)))
        # 距离因子：越接近通信半径成功率越低
        d = self.seg_length(c, a)
        dist_factor = max(0.0, 1.0 - d / (self.cfg.comm_radius * 1.5))
        # 速度/链路存活因子：相对速度越大，链路越不稳定
        vc = self.node_speed.get(c, self.cfg.veh_speed)
        va = self.node_speed.get(a, self.cfg.veh_speed)
        rel_v = abs(vc - va)
        speed_factor = math.exp(-rel_v / 30.0)
        # 组合，并限制 [0,1]
        p = dens_factor * dist_factor * speed_factor
        return max(0.0, min(1.0, p))

    def _queue_delay(self, node: int) -> float:
        q = self.node_queue.get(node, 0.0)
        return q / max(1e-3, self.cfg.queue_depart_rate)
