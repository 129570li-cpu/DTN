...
class SDVNController:
    ...
    def select_policy_name(self, load_level=1, message_type="efficiency"):
        # Determine policy via APN state-action table when available
        try:
            action = self.state_action.select((message_type, load_level))
        except Exception:
            action = None
        if action is None:
            tag = getattr(Gp, "tag", 0)
            if tag == 1:
                action = "HRF"
            elif tag == 2:
                action = "LDF"
    ...
    # GNN 路径计算，按策略选择不同评分头
    def calculate_area_path(self, node_id, des_id, policy_name=None):
        if not (self.gnn_router_enabled and self.gnn_router):
            raise RuntimeError("GNN router not initialized.")
        policy = policy_name or self.select_policy_name()
        gnn_path = self._calculate_area_path_gnn(node_id, des_id, policy)
        if not gnn_path:
            print(f"[SDVN] GNN router failed for request node={node_id} dst={des_id} policy={policy}")
            return []
        return gnn_path

    def get_area_id(self, node_id: int) -> int:
        try:
            val = self.it_cover.get(node_id)
            if isinstance(val, (list, tuple)) and len(val) > 0:
                return int(val[0])
            if isinstance(val, int):
                return int(val) if val >= 0 else int(node_id)
        except Exception:
            pass
        return int(node_id)

    def _calculate_area_path_gnn(self, node_id, des_id, policy_name: str):
        try:
            src_area = self.get_area_id(node_id)
            dst_area = self.get_area_id(des_id)
            if not self.gnn_router.has_node(src_area) or not self.gnn_router.has_node(dst_area):
                print(f"[SDVN] Invalid area id for GNN src={src_area} dst={dst_area}")
                return []
            path = self.gnn_router.compute_path(src_area,
                                                dst_area,
                                                policy_name=policy_name,
                                                max_hops=len(self.intersection) + 5)
            if not path:
                print(f"[SDVN] Empty path returned by GNN src={src_area} dst={dst_area}")
            return path
        except Exception as exc:
            print(f"[SDVN] Exception during GNN routing: {exc}")
            return []
    ...
    def resolve_request(self, node_list):
        ...
        for request in self.flow_request_list: # flow_request_list由Node中产生路由请求的generate_request()生成
            # 查表计算路径
            policy_name = self.select_policy_name()
            area_path = self.calculate_area_path(request.node_id, request.des_id, policy_name=policy_name)
            # 向发送请求节点回复
    ...
    def resolve_report(self):
        ...
        for report in self.flow_report_list:
            try:
                ...
                action = self.select_policy_name(load_level=load_level, message_type=message_type)
                # 构造逐段几何近似指标
