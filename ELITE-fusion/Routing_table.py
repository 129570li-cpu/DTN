...
class Routing_Table:
    ...
    def fusion_weight(self):
        ...
        # print summary instead of full table to avoid heavy stdout
        try:
            print("table_BP: shape=", self.table_BP.shape)
        except Exception:
            pass
        #exit(0)
        return
    ...
    def fusion_fuzzy(self):
        def _fallback_val(df):
            return 0.0 if df is self.table_PRR_norm else 1.0
        def _scalar(df, cur, neib, des):
            try:
                val = df.loc[(cur, neib), des]
            except Exception:
                # fallback to chained indexing
                try:
                    val = df[des][cur][neib]
                except Exception:
                    return _fallback_val(df)
            try:
                return float(val)
            except Exception:
                try:
            ...
            for neib in neibs:
                ...
                for des in Gp.it_pos:
                    if des in neibs or cur == des:
                        ...
                        v_prr = _scalar(self.table_PRR_norm, cur, neib, des)
                        v_ad  = _scalar(self.table_AD_norm,  cur, neib, des)
                        v_rc  = _scalar(self.table_RC_norm,  cur, neib, des)
                        fusion_result = PFR.fuse_hrf(v_prr, v_ad, v_rc)
                        row.append(fusion_result)
                matrix.append(row)
        self.table_HRF = self.table_config(matrix)
        try:
            print("table_HRF: shape=", self.table_HRF.shape)
        except Exception:
            pass
        # 2
        matrix = []
            ...
            for neib in neibs:
                ...
                for des in Gp.it_pos:
                    if des in neibs or cur == des:
                        ...
                        v_ad  = _scalar(self.table_AD_norm,  cur, neib, des)
                        v_hc  = _scalar(self.table_HC_norm,  cur, neib, des)
                        v_prr = _scalar(self.table_PRR_norm, cur, neib, des)
        ...
        try:
            print("table_LDF: shape=", self.table_LDF.shape)
        except Exception:
            pass
        # 3
        matrix = []
        ...
        try:
            print("table_LBF: shape=", self.table_LBF.shape)
        except Exception:
            pass
        return
